...
func distinctRoomTypes(rooms []schedule.Room) []string {
...
func roomFromForm(r *http.Request) schedule.Room {
...
func (h *Handler) reRoomsForm(w http.ResponseWriter, r *http.Request, status int, rm schedule.Room, isEdit bool, errMsg string) {
	data, err := h.input.LoadInput(r.Context())
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
