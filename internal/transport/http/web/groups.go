...
func groupFromForm(r *http.Request) schedule.Group {
...
func (h *Handler) reGroupsForm(w http.ResponseWriter, r *http.Request, status int, g schedule.Group, isEdit bool, errMsg string) {
	data, err := h.input.LoadInput(r.Context())
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
