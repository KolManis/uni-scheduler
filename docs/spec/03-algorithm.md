...
Алгоритм построения выбирается параметром `solver_type` (ADR-0015):

| Значение | Построение |
|---|---|
| `teacher`, пусто | по преподавателям — пары идут блоками по преподавателю |
| `dsatur` | самая трудная пара первой (DSatur) — порядок пересчитывается после каждой пары |
