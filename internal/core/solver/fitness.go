...
func CalculateFitnessBreakdown(assignments []domain.Assignment, _ domain.InputData) domain.FitnessBreakdown {
	...
	// 7. Штраф за одну пару в день у группы: ехать в универ ради одной пары —
	// потерянный день, студенты этого сильно не любят. Штраф сопоставим со штрафом за
	// одинокую субботу (8000) — то же явление, только в будни: если алгоритм видит
	// возможность объединить эту одиночную пару с другими днями, он должен её выбрать.
	for _, daySlots := range groupSlots {
		for _, slots := range daySlots {
			if len(slots) == 1 {
				b.SingleClassDay += 8000
			}
		}
	}
