...
func orderTeachersByDifficulty(input domain.InputData) []domain.Teacher {
	teachers := make([]domain.Teacher, len(input.Teachers))
...
func generateCompactBlocks(state *teacherState, teacher domain.Teacher, subjects []domain.SubjectPlan) [][]domain.TimeSlot {
	...
	sort.Slice(allSlots, func(i, j int) bool {
		if allSlots[i].Day() == domain.Saturday && allSlots[j].Day() != domain.Saturday {
			return false
		}
		if allSlots[i].Day() != domain.Saturday && allSlots[j].Day() == domain.Saturday {
			return true
	...
	daySlots := make(map[domain.Day][]domain.TimeSlot)
	for _, slot := range allSlots {
		daySlots[slot.Day()] = append(daySlots[slot.Day()], slot)
	}

	var blocks [][]domain.TimeSlot
	slotsLeft := remaining
	for _, day := range []domain.Day{domain.Monday, domain.Tuesday, domain.Wednesday, domain.Thursday, domain.Friday, domain.Saturday} {
		if slotsLeft <= 0 {
			break
		}
...
func findSlotInBlock(state *teacherState, teacher domain.Teacher, plan domain.SubjectPlan, block []domain.TimeSlot) (*domain.TimeSlot, *domain.Room, float64) {
