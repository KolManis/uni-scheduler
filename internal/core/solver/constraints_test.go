...
func TestIsSlotFree_EmptyOccupied(t *testing.T) {
	...
	if !isSlotFree(slot, "T1", domain.Always, occupied) {
...
func TestIsSlotFree_AlwaysConflict(t *testing.T) {
	slot := domain.MustNewTimeSlot(domain.Monday, 1)
	occupied := map[domain.TimeSlot]map[string]domain.Parity{
		slot: {"T1": domain.Always},
	}
	if isSlotFree(slot, "T1", domain.Even, occupied) {
...
func TestIsRoomBigEnough_OverCapacity(t *testing.T) {
	room := domain.Room{ID: "R1", Capacity: 20}
	groupMap := map[string]domain.Group{
		"G1": {StudentCount: 25},
	}
