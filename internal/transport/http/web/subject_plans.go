...
func (h *Handler) subjectPlansForm(w http.ResponseWriter, r *http.Request) {
	...
	fd.Plan.Parity = schedule.Always
	fd.Plan.SemesterHalf = schedule.HalfFull
...
func subjectPlanFromForm(r *http.Request) schedule.SubjectPlan {
...
func (h *Handler) reSubjectPlansForm(w http.ResponseWriter, r *http.Request, status int, sp schedule.SubjectPlan, isEdit bool, errMsg string) {
	data, err := h.input.LoadInput(r.Context())
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
