...
type schedulesListData struct {
	Schedules []repo.ScheduleSummary
	Error     string
	Success   string
}
...
type scheduleViewData struct {
	...
	DayGroups    []dayGroup
	Breakdown    solver.FitnessBreakdown
...
func buildGroupBlocks(sched *schedule.Schedule, groups []schedule.Group) []groupBlock {
	type key struct {
		day  schedule.Day
		pair int
	}
	...
	sortedGroups := make([]schedule.Group, len(groups))
	...
	for _, g := range sortedGroups {
		...
		dayMin := map[schedule.Day]int{}
		dayMax := map[schedule.Day]int{}
		for k := range cells {
...
func buildDayGroups(sched *schedule.Schedule) []dayGroup {
	byDay := map[schedule.Day]map[int][]assignmentView{}
	for idx, a := range sched.Assignments {
...
func (h *Handler) schedulesGenerate(w http.ResponseWriter, r *http.Request) {
	...
	in := generator.GenerateInput{
		...
		SemesterHalf: schedule.SemesterHalf(r.FormValue("semester_half")),
	}
...
func (h *Handler) loadScheduleView(w http.ResponseWriter, r *http.Request, id int64, status int, errMsg, successMsg string) {
	...
	renderStatus(w, r, status, h.pages["schedules_view.html"], scheduleViewData{
		...
		Breakdown: solver.CalculateFitnessBreakdown(sched.Assignments, *data),
...
func conflictMessage(c *generator.ConflictError) string {
	var what string
	switch c.Type {
	case "teacher_busy":
...
func (h *Handler) reAssignmentForm(w http.ResponseWriter, r *http.Request, id int64, idx int, req generator.PatchRequest, status int, errMsg string) {
	sched, err := h.svc.GetByID(r.Context(), id)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
...
func (h *Handler) schedulesPatchAssignment(w http.ResponseWriter, r *http.Request) {
	...
	r.ParseForm()
	req := generator.PatchRequest{
		TimeSlot: schedule.TimeSlot{Day: schedule.Day(r.FormValue("day")), PairNum: atoi(r.FormValue("pair_num"), 0)},
		RoomID:   r.FormValue("room_id"),
		Parity:   schedule.Parity(r.FormValue("parity")),
	}
